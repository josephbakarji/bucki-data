...
hyperparams = {
'nsamp': [500],
'noise_amp':[1e-06, 1e-05, 5e-05, 0.0001, 0.0005, 0.001, 0.005, 0.01, 0.05, 0.1, 
...
savedir = './results/'
savefile = 'blasius_buckinet_hyper_noise.json'
data_list = []
...
for hyperp in hyperparams_list:
    ...
    outputs = q[0, idx].T 
    outputs += params['noise_amp'] * np.random.normal(loc=0, scale=1.0, size=outputs.shape) 
    inputs = p[idx, :]
    ...
    try:
        ...
        data = {'x': x.T.tolist(), 'loss': loss, 'params': params}
